    ...
    switch (action.type) {
        ...
        case 'set_carts' :
            return {
                ...
                carts: {
                    data:
                        state.carts.data.filter((el) => el.id == action.payload.id && el.size === action.payload.size).length

                        ? state.carts.data.map((el) => el.id == action.payload.id && el.size === action.payload.size ? {...el, count : el.count + 1} : el )
...
const Context = (props) => {
    ...
    const getProducts = () => {
            axios(`http://localhost:4444/catalog?gender=${state.catalog.gender}&category=${state.catalog.category}${state.catalog.price !== '' ? '&_sort=price&_order=' + state.catalog.price : ''}${state.catalog.brand !== '' ? '&brand=' + state.catalog.brand : ''}`)
                .then(({data}) => {
